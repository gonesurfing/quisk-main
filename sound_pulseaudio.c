...
{
   pa_sample_spec ss;
   pa_buffer_attr ba;		// buffer attributes added by Philip Lee
   int error = 0;
   ...
   ss.channels = dev->num_channels;
	...
	// Construct default buffer attributes; added by Philip Lee
	ba.maxlength = -1; 
	ba.tlength = -1; 
	ba.prebuf = -1; 
	ba.minreq = -1; 
	...
	// Adjust target buffer length according to latency_millisecs
	if( direction == PA_STREAM_PLAYBACK )
		ba.tlength = sizeof(float) * ss.channels * ss.rate / 1000 * quisk_sound_state.latency_millisecs;
...
{	// Return a list of PulseAudio device names [pycapt, pyplay]
	...
	if (pa_context_connect(pa_ctx, NULL, 0, NULL) < 0)		// No local daemon
	   return pylist;
